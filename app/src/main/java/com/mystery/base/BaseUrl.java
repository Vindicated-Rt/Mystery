package com.mystery.base;

/**
 * Created by Vindicated-Rt
 * 2020-02-13 14:45
 * 全局静态地址变量
 */
public class BaseUrl {
    public static final String ACTIVITY_URL_SPLASH = "/activities/splash";
    public static final String ACTIVITY_URL_HOME = "/activities/home";
    public static final String ACTIVITY_URL_LUNCH = "/activities/lunch";
    public static final String ACTIVITY_URL_LOGIN = "/activities/login";
    public static final String ACTIVITY_URL_REGISTER = "/activities/register";
    public static final String ACTIVITY_URL_FORGET = "/activities/forget";
}
